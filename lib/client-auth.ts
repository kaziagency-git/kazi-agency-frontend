const ACCESS_KEY = 'kazi_client_access';
const REFRESH_KEY = 'kazi_client_refresh';
...
export function setClientTokens(accessToken: string, refreshToken: string): void {
  setCookie(ACCESS_KEY, accessToken, 60 * 60);
...
export function hasValidClientRefreshToken(): boolean {
  const token = getClientRefreshToken();
  return !!token && !isTokenExpired(token);
}
