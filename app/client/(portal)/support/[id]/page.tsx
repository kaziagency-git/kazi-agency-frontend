...
import { getMyTicket, replyToTicket, uploadFiles, Ticket, TicketAttachment } from '@/lib/client-api';
...
export default function TicketDetailPage() {
  ...
  async function handleReply(e: React.FormEvent) {
    ...
    try {
      ...
      const updated = await replyToTicket(id, reply, attachments);
      setTicket(updated);
      setReply('');
      setFiles([]);
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
              ...
              {ticket!.messages.map((msg) => (
                ...
                >
                  ...
                  <div className={`max-w-[75%] space-y-1 ${msg.sender === 'client' ? 'items-end' : 'items-start'} flex flex-col`}>
                    {msg.content && (
                      <div
                        className={`rounded-2xl px-4 py-3 text-sm prose prose-sm max-w-none ${msg.sender === 'admin' ? 'bg-muted rounded-tl-sm' : 'bg-primary text-primary-foreground rounded-tr-sm prose-invert'}`}
      ...
      {!isClosed ? (
        ...
        <p className="text-center text-sm text-muted-foreground">This ticket is closed. Open a new one if you need further help.</p>
      )}
    </div>
  );
}
